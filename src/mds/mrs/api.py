...
CONTENT_TYPES = {
    ...
    'SOUND': 'audio/3gpp',
    'VIDEO': 'video/3gpp',
    'BINARYFILE': 'image/jpeg',
    'PLUGIN':'application/java-archive'}
...
"""Month name to two character numeric string mapping. This includes the Portuguese
month names.
"""

MALE = ['Male', 'male', 'M', 'm', 'Masculino',]
"""Recognized strings for male gender"""
FEMALE = ['Female', 'female', 'F', 'f','Feminimo',]
"""Recognized strings for female gender"""
...
def register_saved_procedure(sp_guid, procedure_guid, responses,
    ...
    '''
    logging.info('(Client: %s,  Encounter: %s) => Registering' % (client_id,sp_guid))
    ...
    # BINARY_TYPES above
    try:
        responses_dict = cjson.decode(responses)
        for k,v in responses_dict.items():
            ...
            if v['type'] in BINARY_TYPES:
                ...
                items = answer.split(',')
                if settings.LOG_DEBUG:
                    logging.debug("Found File type: %s : %s" % (k,v))
                    logging.debug("answer, len(answer): %s, %d" % (answer, len(items)))
                for item in items:
                    if not item:
                        logging.debug("Binary type with empty answer: %s" % v['answer'])
                        continue
                    br,created = BinaryResource.objects.get_or_create(
                                        procedure=sp, element_id=k, guid=item)
                    br.total_size = 0
                    mime_type = v.get('mimeType', None)
                    content_type = mime_type if mime_type else CONTENT_TYPES[v['type']]
                    if settings.LOG_DEBUG:
                        logging.debug("Content Type(element_id,type, mime): (%s, %s, %s)" % (k,v['type'], content_type))
...
def convert_binary(binary):
    ...
    ftype = CONVERTED_CONTENT_TYPES[file_type]
    logging.debug("CONVERSION ARGS: %s %s %s " % (binary, ftype, extension))
    result, message = FFmpeg()(binary, ftype, extension)
