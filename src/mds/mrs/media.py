...
FFMPEG_EXTRA_ARGS = {'SOUND': ['-ar', '44100','-acodec', 'libmp3lame'],         
...
def convert_binary(binary):
    ...
    """
    result = False
    ftype = binary.content_type
    converter = get_converter(ftype)
    extension = get_extension(ftype)
    result = converter(binary, ftype, extension)
    return result